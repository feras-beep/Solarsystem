...
export default function CompleteSolarSystemExplorer() {
  ...
  useEffect(() => {
    ...
    const onMouseClick = (e) => {
      ...
      if (moveDistance < 5 && timeDiff < 300) { // Less than 5px movement and under 300ms
        ...
        // Check intersections with celestial bodies
        const meshes = Object.entries(celestialBodies.current).map(([key, body]) => ({
          mesh: body.mesh,
          key: key
        }));
        ...
        if (intersects.length > 0) {
          ...
          const clickedBody = meshes.find(m => m.mesh === clickedMesh);
          
          if (clickedBody) {
            setSelectedBody(clickedBody.key);
    ...
    const onMouseHover = (e) => {
      // Check if mouse is over a planet to show pointer cursor
      const rect = renderer.domElement.getBoundingClientRect();
      mouse.x = ((e.clientX - rect.left) / rect.width) * 2 - 1;
      mouse.y = -((e.clientY - rect.top) / rect.height) * 2 + 1;

      raycaster.setFromCamera(mouse, camera);
      const meshes = Object.values(celestialBodies.current).map(body => body.mesh);
    ...
    const animate = () => {
      ...
      if (!isPausedRef.current) {
        time += 0.001 * timeScaleRef.current;
        ...
        Object.entries(celestialBodies.current).forEach(([key, body]) => {
          ...
          if (data.rotationPeriod !== 0) {
            const rotationSpeed = (2 * Math.PI / Math.abs(data.rotationPeriod)) * 0.01 * timeScaleRef.current;
            body.mesh.rotation.y += data.rotationPeriod > 0 ? rotationSpeed : -rotationSpeed;
          }

          // Orbital motion
          if (data.orbitalPeriod > 0 && !data.parent) {
            body.angle += (2 * Math.PI / data.orbitalPeriod) * 0.01 * timeScaleRef.current;
            ...
            const orbitSpeed = (2 * Math.PI / Math.abs(data.orbitalPeriod)) * 0.01 * timeScaleRef.current;
      ...
      if (cameraModeRef.current === 'free') {
        ...
        camera.lookAt(0, 0, 0);
      } else if (cameraModeRef.current === 'follow' && celestialBodies.current[selectedBodyRef.current]) {
        const body = celestialBodies.current[selectedBodyRef.current];
      ...
      } else if (cameraModeRef.current === 'orbit' && celestialBodies.current[selectedBodyRef.current]) {
        const body = celestialBodies.current[selectedBodyRef.current];
        const worldPos = new THREE.Vector3();
        body.mesh.getWorldPosition(worldPos);
        
        const radius = body.data.radius * 4;
        const speed = 0.001 * timeScaleRef.current;
